...
def chunk_input(input_tensor, WeightChunks,ds):
    input_channels, height, width = input_tensor.shape
    chunk_size = input_channels // WeightChunks
    input_chunks = []

    for i in range(WeightChunks):
        start_index = i * chunk_size
        end_index = input_channels if i == WeightChunks - 1 else (i + 1) * chunk_size
        input_chunk = input_tensor[start_index:end_index, :, :]
        fmt_mem = ds.reorder_mat(input_chunk, "YCXC8", "CYX")
        input_chunks.append(fmt_mem)
    reordered_chunks=np.concatenate((input_chunks), axis=None)
    return reordered_chunks

def input_split(input_tensor, WeightChunks,ds):
    C = input_tensor.shape[0]
    C_half = C // WeightChunks
    core1_tensor = input_tensor[:C_half, :, :]  # Shape: [32, 7, 7]
    core2_tensor = input_tensor[C_half:, :, :]  # Shape: [32, 7, 7]
    reordered_chunks = []
    core1_tensor_mem_fmt = ds.reorder_mat(core1_tensor, "YCXC8", "CYX")
    core2_tensor_mem_fmt = ds.reorder_mat(core2_tensor, "YCXC8", "CYX")
    reordered_chunks=np.concatenate((core1_tensor_mem_fmt,core2_tensor_mem_fmt), axis=None)
    print(reordered_chunks.shape)
    return reordered_chunks
