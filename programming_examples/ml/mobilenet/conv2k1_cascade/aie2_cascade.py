...
# HERE
            ...
            conv2dk1_skip_get = external_func(
                "conv2dk1_skip_ui8_i8_i8_get",
                inputs=[
                    ty_in,
                    ty_wts,
                    ty_out,
            ...
            conv2dk1_skip_put = external_func(
                "conv2dk1_skip_ui8_i8_put",
                inputs=[
                    ty_in,
                    ty_wts,
                    int32_ty,
            ...
            object_fifo_link(OF_wts_L3L2, [OF_wts_memtile_put,OF_wts_memtile_get])
            ...
            OF_bn_12 = object_fifo("OF_bn_12", ComputeTile04, [MemTile01], 2, ty_out)
            OF_outOFL2L3 = object_fifo("outOFL2L3", MemTile01, [ShimTile00], 2, ty_out)
            object_fifo_link(OF_bn_12, OF_outOFL2L3)


            # Compute tile 4
            @core(ComputeTile05, "conv2dk1_skip_put.o")
            def core_body():
                ...
                for _ in for_(0xFFFFFFFF):
                    ...
                    for _ in for_(InH2):
                        ...
                        # elemOut0 = OF_bn_12.acquire(ObjectFifoPort.Produce, 1)
                        # elementSkipsIn = OF_skip.acquire(
                        ...
                        # objectfifo_release(ObjectFifoPort.Produce, "OF_bn_12", 1)
                        # objectfifo_release(ObjectFifoPort.Consume, "OF_skip", 1)
                        yield_([])
                    objectfifo_release(ObjectFifoPort.Consume, "OF_wts_memtile_put", 1)
                    yield_([])
            
            # Compute tile 4
            @core(ComputeTile04, "conv2dk1_skip_get.o")
            def core_body():
            ...
            acitivationsOutSize32b = (InW1 * InW1 * OutC) // 4
